...
if (users.length > 0) {
  const decryptedUsers = users.map(user => {
    ...
    try {
      // Kiểm tra xem dữ liệu có đúng format mã hóa không
      if (!plainUser.name?.includes(':') || !plainUser.email?.includes(':')) {
        // Nếu dữ liệu chưa được mã hóa, trả về dữ liệu gốc
        return {
          ...plainUser,
          name: plainUser.name || '',
      ...
      return {
        ...plainUser,
        name: decryptData(plainUser.name),
        email: decryptData(plainUser.email)
      };
    } catch (error) {
      console.error('Decryption error for user:', plainUser._id, error);
      // Nếu không giải mã được, trả về dữ liệu gốc
      return {
        ...plainUser,
        name: plainUser.name || '',
        email: plainUser.email || ''
  ...
  const isEmailExist = decryptedUsers.some(user => user.email === email);
  if (isEmailExist) {
    return next(new ErrorHandler("Email đã tồn tại", 400));
  }
}
...
export const activateLoginUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { email, password } = newUser.user as ILoginRequest;
     
      // Lấy tất cả users để tìm kiếm email
  const users = await userModel.find({}).select("+password");
  const decryptedUsers = users.map(user => {
    const plainUser = user.toObject();
    try {
      // Kiểm tra và giải mã dữ liệu
      if (!plainUser.name?.includes(':') || !plainUser.email?.includes(':')) {
        return {
          ...plainUser,
          name: plainUser.name || '',
  ...
  if (!user) {
    return next(new ErrorHandler("Email hoặc mật khẩu không hợp lệ", 400));
      ...
      sendToken(userInstance, 200, res);
...
if (users.length > 0) {
  ...
  const userInstance = new userModel(user);
  // Kiểm tra password
  const isPasswordMatch = await userInstance.comparePassword(password);
  if (!isPasswordMatch) {
    return next(new ErrorHandler("Email hoặc mật khẩu không hợp lệ", 400));
  }

  // Nếu mọi thứ ok, tiếp tục xử lý đăng nhập
  // ...rest of your login logic
}

      const userLogin: ILoginRequest = {
        email,
        password,
      };

      const activationToken = createActivationToken(userLogin);

      const activationCode = activationToken.activationCode;

      const data = { user: { name: userLogin.email }, activationCode };
      const html = await ejs.renderFile(
        path.join(__dirname, "../mails/activation-mail.ejs"),
        data
      );
      ...
      try {
        ...
        return res.status(201).json({
          success: true,
          message: `Vui lòng kiểm tra tài khoản của bạn: ${userLogin.email} để xác thực tài khoản!`,
          activationToken: activationToken.token,
        });
      } catch (error: any) {
        return next(new ErrorHandler(error.message, 400));
...
export const updateProfilePicture = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      userInstance.name = decryptData(userInstance.name)
      userInstance.email = decryptData(userInstance.email)

      console.log("UPDATE ANH:"+userInstance)
      await redis.set(userId, JSON.stringify(userInstance));
      console.log(userInstance?.avatar.url);
      res.status(200).json({
        success: true,
        userInstance,
      });
